...
#' @S3method $ quoted
`$.quoted` <- function(x, name) {
  structure(enquote(unclass(x)[[name]]), env = attr(x, "env"), class = "quoted")
}
...
#' @S3method as.quoted list
#' @export
as.quoted.list <- function(x,env=parent.frame()) {
  do.call(c,lapply(x,as.quoted))
}

#' Generic version of all.vars
#' @export
all.vars <- function(expr,...){
  UseMethod("all.vars")
}

#' @S3method all.vars default
#' @export
all.vars.default <- function(expr,...){
  base::all.vars(expr,...)
}
...
#' Generic version of all.names
#' @export
all.names <- function(expr,...){
  UseMethod("all.names")
}

#' @S3method all.names default
#' @export
all.names.default <- function(expr,...){
  base::all.names(expr,...)
}

#' @S3method all.names quoted
#' @export
all.names.quoted <- function(expr,...,unique=TRUE) {
  vars = unlist(lapply(expr,all.names,...,unique=unique))
  if (unique) {
    unique(vars)
  }else {
    vars
